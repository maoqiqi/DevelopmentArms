...
public class TasksActivity extends BaseActivity implements View.OnClickListener {
    ...
    public static final String TASKS_VIEW_MODEL = "TASKS_VIEW_MODEL";
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // In a configuration change we might have a ViewModel present. It's retained using the Fragment Manager.
        @SuppressWarnings("unchecked")
        ViewModelHolder<TasksViewModel> viewModelHolder = (ViewModelHolder<TasksViewModel>) getSupportFragmentManager().findFragmentByTag(TASKS_VIEW_MODEL);
        if (viewModelHolder != null && viewModelHolder.getViewModel() != null) {
            // If the model was retained, return it.
            tasksViewModel = viewModelHolder.getViewModel();
        } else {
            // There is no ViewModel yet, create it.
            tasksViewModel = new TasksViewModel(Injection.provideTasksRepository(this));
            // and bind it to this Activity's lifecycle using the Fragment Manager.
            getSupportFragmentManager().beginTransaction().add(ViewModelHolder.createContainer(tasksViewModel), TASKS_VIEW_MODEL).commit();
        }
